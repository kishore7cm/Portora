#!/usr/bin/env python3
"""
Fetch REAL market data using actual Alpaca API for stocks and Twelve Data API for crypto
"""
...
def get_stock_price_alpaca(ticker):
    ...
    try:
        ...
        if 'quote' in data and data['quote']:
            return float(data['quote']['ap'])  # 'ap' is the ask price
        else:
...
def get_historical_stock_price_alpaca(ticker, date):
    ...
    try:
        ...
        if 'bars' in data and data['bars'] and ticker in data['bars']:
            ...
            print(f"Could not get Alpaca historical price for {ticker} on {date}")
            return None
    except Exception as e:
...
def get_historical_crypto_price_twelve(ticker, date):
    ...
    try:
        # Twelve Data API endpoint for historical data
        ...
        if 'values' in data and data['values']:
            ...
            print(f"Could not get Twelve Data historical price for {ticker} on {date}")
            return None
    except Exception as e:
...
def fetch_real_market_data():
    """Fetch REAL market data for the past week using actual APIs"""
    print("🚀 Fetching REAL market data using Alpaca (stocks) and Twelve Data (crypto) APIs...")
    ...
    try:
        ...
        # Process only first 10 holdings for testing (to avoid API rate limits)
        ...
        for i, holding in enumerate(test_holdings):
            ...
            if is_crypto:
                ...
                historical_price = get_historical_stock_price_alpaca(ticker, sep_23_2025)
        ...
        print(f"📊 This data is based on actual API calls to Alpaca and Twelve Data")
        print(f"⚠️  Note: Only processed first 10 holdings to avoid API rate limits")
...
if __name__ == "__main__":
    print("Fetching REAL market data using Alpaca and Twelve Data APIs...")
