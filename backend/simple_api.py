"""
Simple working API server for portfolio data
"""
...
# Sample portfolio data - Updated to reflect ~$330k current value
SAMPLE_PORTFOLIO_DATA = {
    "user_id": 1,
    "date": "2025-10-02",
    "total_value": 342997.19,
    "cash": 15000.0,
    "positions": [
...
DASHBOARD_DATA = {
    ...
    "current_value": 342997.19,
    "time_period_days": 9,
    "net_worth": 342997.19,
    "total_gain_loss": 17997.19,
    "return_pct": 5.54,
    "allocation": {
        "stock": 72.5,
        "bond": 9.0,
    ...
    "top_holdings": [
        {"ticker": "ETH", "units": 15.0, "price": 4300.0, "position_val": 64500.0},
        {"ticker": "BTC", "units": 0.5, "price": 115000.0, "position_val": 57500.0},
        {"ticker": "META", "units": 90.0, "price": 580.0, "position_val": 52200.0}
    ],
    "movers": {"up": [], "down": []},
    "risk": {"sharpe_ratio": 1.2, "volatility_annualized": 18.5},
    "diversification": {"score": 85, "risk_level": "Medium"},
    "health_score": 88,
    "missing_prices": []
}
...
@app.get("/")
def root():
    return {"message": "Simple Portfolio API is running", "status": "healthy"}
...
def get_portfolio_legacy(user_id: int):
    ...
    return {
        ...
        "summary": {
            ...
            "Total_Cost_Basis": 325850.92,
            "Total_Gain_Loss": 4149.08,
            "Total_Gain_Loss_Percent": 1.27,
...
def get_insights(user_id: int):
    ...
    try:
        ...
        result = insights_service.generate_insights(user_id)
        return {
            "user_id": user_id,
        ...
        return {
            "user_id": user_id,
            "insights": [
                "Your portfolio shows steady growth with a balanced risk profile.",
                "Consider rebalancing if any single position exceeds 20% of total value.",
                "Regular monitoring and gradual adjustments can optimize long-term returns."
            ],
            "source": "fallback",
            "disclaimer": "AI insights generated automatically. Not financial advice.",
            "generated_at": "2025-10-02T09:00:00Z",
